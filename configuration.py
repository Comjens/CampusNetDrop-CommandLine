...
if configure == "yes" or configure == "y":
    ...
    print("Configure default Path? (yes/no)")
    pathy = input()
    if pathy == "yes" or pathy == "y":
        print("Where to download the folders? (Path)")
        path = input().replace("\\","/")
    ...
    for node in root:
        for child in node:
            ...
            if answer=="yes" or answer=="y":
                print("Use default path for \"%s --- %s\" " % (node.get('Name'),child.get('Name')))
                answer2 = input()
                if answer2=="yes" or answer2=="y":
                    f.write(child.get('Name')+";"+child.get('Id')+";"+path+"/"+child.get('Name').replace(" ","_")+"\n")
                else:
